...
function Navbar() {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            if (creditPopupRef.current && !creditPopupRef.current.contains(event.target)) {
    ...
    const handleLogout = async () => {
        try {
            ...
            setError(err.response?.data?.message || "Failed to logout. Please try again.")
        }
    }
            ...
            <div className="flex justify-center px-4">
                ...
                >
                    ...
                    window.scrollTo(0, 0);
                        ...
                        <h2 className="font-semibold hidden md:block text-lg text-emerald-800">MockHire AI</h2>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative hidden md:block" ref={creditPopupRef}>
                            ...
                            >
                                ...
                                <span className="font-semibold text-emerald-700">{user?.credits || 0}</span>
                            </button>
                            <AnimatePresence>
                                {showCreditPopup && (
                                    <motion.div
                        ...
                        {user ? (
                            <div className="relative" ref={userPopupRef}>
                                ...
                                >
                                    {user?.name ? user.name.slice(0, 1).toUpperCase() : <FaUserAstronaut size={18} />}
                                </button>
                                <AnimatePresence>
                                    {showUserPopup && (
                                        <motion.div
                                        ...
                                        >
                                            <div className="border-b border-gray-100 pb-3 mb-3">
                                                <p className="text-lg font-bold text-emerald-700">{user?.name}</p>
                            ...
                            <div className="flex items-center gap-3">
                                <button onClick={() => setShowAuthModel(true)}
                        ...
                        <button className="md:hidden cursor-pointer" onClick={() => setShowMobileMenu(!showMobileMenu)}>
                            {showMobileMenu ? <HiX size={28} className="text-emerald-700" /> : <HiMenu size={28} className="text-emerald-700" />}
                        </button>
                    </div>
                                    ...
                                    {user && (
                                        <>
                                            <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-100 rounded-lg">
                                                <BsCoin size={20} className="text-emerald-600" />
                                                <span className="font-semibold text-emerald-700">{user?.credits} Credits</span>
                                            </div>
                                            <button onClick={() => {
                                                navigate("/history")
                                                setShowMobileMenu(false)
